...
function AuthModal({ isOpen, onClose, initialMode = 'signin' }: { isOpen: boolean; onClose: (open: boolean) => void; initialMode?: 'signin' | 'signup' }) {
  ...
  const variants = {
    initial: (direction: number) => ({
      x: direction > 0 ? 100 : -100, opacity: 0, position: "absolute" as const,
    }),
    animate: {
      x: 0, opacity: 1, position: "relative" as const, transition: { duration: 0.4, ease: easeInOut },
    },
    exit: (direction: number) => ({
      x: direction < 0 ? 100 : -100, opacity: 0, position: "absolute" as const, transition: { duration: 0.4, ease: easeInOut },
    }),
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="relative overflow-hidden bg-slate-950/90 backdrop-blur-sm border-slate-800 text-white min-h-[420px] flex flex-col justify-center fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <AnimatePresence mode="wait" custom={isSignIn ? 1 : -1}>
          <motion.div
            key={isSignIn ? "signIn" : "signUp"} variants={variants} initial="initial"
            animate="animate" exit="exit" custom={isSignIn ? 1 : -1} className="w-full"
          >
            <DialogHeader>
            ...
            <p className="text-center text-sm text-slate-400 mt-3">
              ...
              <Button variant="link" onClick={toggleMode} className="pl-1.5 text-blue-400 hover:text-blue-300">
...
export default function LandingPage() {
  ...
  const { user, isLoading } = useAuth(); // Get user state from context

  const handleSignOut = async () => {
    try {
      await signOut(auth); // Sign out from Firebase client
      await fetch('/api/auth/session-logout', { method: 'POST' }); // Destroy server session
    } catch (error) {
